...
<br>
<center><h3>Bảng Khách Hàng</h3></center>
<table class="table ">
    ...
    <tboby>
        <c:forEach items="${listKH}" var="l">
            <tr>
                ...
                <td>${l.matKhau}</td>
                <td>
                    <a href="/khach-hang/detail?id=${l.id}" class="btn btn-primary" role="button" onclick="return confirm('Bạn có muốn xem chi tiết không?')">Detail</a>
                    <a href="/khach-hang/delete?id=${l.id}" class="btn btn-danger" role="button" onclick="return confirm('Bạn có muốn xóa hay không')">Delete</a>
                </td>
            </tr>
        </c:forEach>
    </tboby>
</table>
<button onclick="location.href ='/khachHang/khachHangadd.jsp'">Add</button>
</body>
</html>
